...
describe('absolute URLs', () => {
  ...
  it('should not convert URL', () => {
    const $el = cheerio.load('<a href="http://example.com/bar.html"></a>', null, false);
    assert.equal(
  ...
  it('should convert absolute URL', () => {
    const $el = cheerio.load('<a href="/bar.html"></a>', null, false);
    assert.equal(
      '<a href="http://example.com/bar.html"></a>',
      absolute(path, $el).html()
    );
  ...
  it('should convert relative URL', () => {
    const $el = cheerio.load('<a href="bar.html"></a>', {xmlMode: true}, false);
    assert.equal(
      '<a href="http://example.com/bar.html"></a>',
      absolute(path, $el).html()
    );
  ...
  it('should not throw when encountering invalid URLs', () => {
    const $el = cheerio.load(
      '<html><body><ul><li><a href="mailto:%CAbroken@link.com">Broken link</a></li></ul></body></html>', null, false
    );
    absolute(path, $el);
  });
});
...
describe('absolute URLs with <base> tag', () => {
  ...
  it('should convert relative URL', () => {
    const $el = cheerio.load(head + '<a href="foobar.html"></a>', null, false);
    assert.equal(
